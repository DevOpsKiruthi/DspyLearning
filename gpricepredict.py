...
st.write("Predict the next price range for gold based on current price and market conditions.")
...
with col2:
    ...
    if use_manual_price:
        manual_price = st.number_input("Current Gold Price (USD)", value=float(current_price) if current_price else 1800.0)
        current_price = manual_price
    
    # Market context
    st.text_area("Market Context (editable)", value=get_market_context(), height=200, key="market_context")
    ...
    if st.button("🔍 Predict Price Range", type="primary"):
        if current_price:
            with st.spinner("Analyzing gold market and generating prediction..."):
                try:
                    ...
                    if not gold_data.empty:
                        ...
                        month_change = (gold_data['Close'].iloc[-1] / gold_data['Close'].iloc[-30] - 1) * 100 if len(gold_data) >= 30 else 0
                    ...
                    st.success(f"Prediction Complete for {prediction_timeframe} timeframe")
                    ...
                    st.markdown(f"**Recommendation:** <span style='color:{recommendation_color};font-weight:bold'>{prediction.recommendation}</span>", unsafe_allow_html=True)
            ...
            st.warning("Please ensure gold price data is available or enter a manual price.")

# Historical predictions tracking
st.subheader("📜 Past Predictions")
st.info("This section would track your previous predictions and their accuracy. Feature coming soon!")
...
# Footer
st.markdown("---")
st.caption("Gold Price Range Predictor - Data from Yahoo Finance - Not financial advice")
