...
class CandidateEvaluation(BaseModel):
    name: str
    selected: bool  # Selection decision
    # Flattened fields with constraints
    strengths: List[str]
    weaknesses: List[str]
...
class ResumeEvaluationSignature(dspy.Signature):
    ...
    # Output field using the flattened Pydantic model
    evaluation: CandidateEvaluation = dspy.OutputField(
        desc="Evaluation including candidate's name extracted from resume, selection decision (selected=True/False), and detailed review with strengths, weaknesses, fit score, and comments"
    )
